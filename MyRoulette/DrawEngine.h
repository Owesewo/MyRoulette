...
struct Randomizer
{
	...
	{
		Randomizer::GetNextInt(INT_MIN, INT_MAX);
	}
...
struct DrawEngine
{
	...
	*/
	...
	void SpinRoulette()
	{
		auto x = Randomizer::GetNextInt(0, 36);
		std::cout << "Win " << x << " cell" << std::endl;
		CheckStakes(x);
	...
	void CheckStakes(int winCell)
	{
		...
		int maxRows = wins.capacity() > loses.capacity() ? wins.capacity() : loses.capacity();
